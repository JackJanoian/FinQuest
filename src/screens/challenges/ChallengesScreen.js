    ...
    >
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          >
            <TouchableOpacity 
              style={[styles.claimBadgeButton, { backgroundColor: '#1A237E' }]}
              onPress={onClose}
            >
...
const ChallengeDetailModal = ({ visible, challenge, onClose, badges, onCompleteQuest }) => {
...
const ChallengesScreen = () => {
  ...
  const handleCompleteQuest = (challenge, badge, xpEarned) => {
    ...
    // Update the badge to be earned
    setBadges(badges.map(b => {
      if (b.id === badge.id) {
        // If it's a weekly reset badge, update the lastEarned timestamp
        if (b.weeklyReset) {
          return { ...b, earned: true, lastEarned: now };
        }
    ...
    // Update the challenge to be completed
    setChallenges(challenges.map(c => {
      if (c.id === challenge.id) {
        // If it's a weekly reset challenge, update the lastCompleted timestamp
        if (c.weeklyReset) {
          return { ...c, progress: 100, duration: 'Completed', lastCompleted: now };
        }
        return { ...c, progress: 100, duration: 'Completed' };
      }
      return c;
    }));
    ...
    // Set the selected badge and show the badge earned modal
    setSelectedBadge(badge);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.subtitle}>Complete quests to earn XP and badges</Text>
      </LinearGradient>
